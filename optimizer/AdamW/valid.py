...
class MyAdamW:
    def __init__(self, lr=1e-2, betas=(0.9, 0.999), weight_decay=1e-2):
        self.lr = lr
        self.betas = betas
...
if __name__ == "__main__":
    ...
    model = MyModule()
    optimizer_torch = torch.optim.AdamW(model.parameters(), lr=config['lr'], betas=(config['beta1'], config['beta2']))
    optimizer_mine = MyAdamW(lr=config['lr'], betas=(config['beta1'], config['beta2']))
